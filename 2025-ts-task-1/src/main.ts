...
// 說明：用 Record 表示庫存表。
// 目標：以字串鍵對應到嚴格結構。
export type Inventory = Record<"PLANT-1001" | "PLANT-2001", number>;
...
*/
type UpdateProduct = Pick<Product, "id" | "title"> &
...
const submitProduct: TSubmitProduct = (type, product) => {
  if (type === "create") return (product as CreateProduct).title;
  return (product as UpdateProduct).id;
};
